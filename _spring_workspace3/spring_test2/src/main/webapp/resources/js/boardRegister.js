...
const regExp = new RegExp("\.(exe|sh|bat|daa|jar|mis)$");
...
document.addEventListener('change',(e)=>{
    ...
    if(e.target.id=='files'){ //파일에 변화가 생겼다면property
        // input file element에 저장된 file의 정보가 가져오는 
        ...
        // 여러 파일에 대한검증을 모두 통과하기 위해서
        // * 로 각 파일마다 통과 여부를 확인
        let isOk = 1; //전체 파일 검증 결과
        let ul = `<ul class="list-group list-group-fiush">`;
            for(let file of fileObj){
                ...
                ul += `<div class="mb-3>`;
                ul += `${vaildResult ? '<div class="fw-bold"> 업로드 가능 </div>' : '<div class="fw-bold-danger"> 업로드 불가능 </div>' } `;
                ul += `${file.name}</div>`;
